 ...
 */    
 ...
 {   
    $response = CheckBusinessInFinances($id);
    if ($response['success'] && !$response['exists'])
    { 
        try 
        {    
...
/*
 * Function:    CheckBusinessInFinances
 *
 * Created on Jul 25, 2024
 * Updated on Jul 25, 2024
 *
 * Description: Check if the business exists in the tbl_finances table.
 *
 * In:  $id
 * Out: $response
 *
 */
function CheckBusinessInFinances($id)
{  
    $response = [];
    ...
    { 
        ...
        $query = "SELECT count(0) FROM tbl_finances WHERE bid = $id;";        
        $select = $db->prepare($query);
        $select->execute();        
        $result = $select->fetchColumn();
